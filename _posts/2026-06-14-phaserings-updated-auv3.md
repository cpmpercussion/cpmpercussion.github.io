...
Using PhaseRings as an AUv3 plugin is something I've wanted to use for years and it's been frustrating to see so much progress in AUv3 plugins on iPad (my usual performance rig has been iPad with AUM since 2021) but for PhaseRings to be left behind. The critical point was Pure Data. PhaseRings was a `libpd` app, but Pure Data doesn't like to run as multiple instances. I know there's been some progress here, and an open pull request on `libpd`, but I suddenly remembered that I might be able to use Heavy (`hvcc`) instead to compile the Pd sources into C++. Well it wasn't completely straightforward but eventually I got the Pd patches from PhaseRings into a state where `hvcc` would compile them and they all sound correct. Claude got them integrated back into the app and helped to update the UI with a few modern niceties.
